...
namespace SMS.Areas.Base.Controllers
{
    public class HomeController : BaseController
    {
        ...
        private List<TileData> GetTilesList()
        {
            ...
            lst.Add(new TileData()
            {
                Text = "Intake Performance",
                LandingURL = Url.Action("ApproveIndex", "EmployeeShortLeave", new { area = "Hr" }),
                DataURL = Url.Action("GetUserShortLeaveApprovals", "Home", new { area = "Base" }),
                ColorClass = "tile1",
                IconURL = Url.Content("~/Content/Images/Enrollmentsuccess.png")
            });
            lst.Add(new TileData()
            {
