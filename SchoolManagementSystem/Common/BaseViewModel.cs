...
namespace SMS.Common
{
    public class BaseViewModel<T>
    {
        ...
        public void SetList<E>(IQueryable<E> qry, string DefaultSort, params string[] properties)
        {
            SetList(qry, DefaultSort, SortDirection.Ascending, properties);
        }

        public void SetList<E>(IQueryable<E> qry, string DefaultSort, SortDirection DefaultSortDir = SortDirection.Ascending, params string[] properties)
        {
            var startPage = 0;
            ...
            Func<E, T> GetModelObject = (x) =>
            {
                if (properties.Count() == 0)
                    ...
                    return (T)Activator.CreateInstance(typeof(T), x, properties);
            };
