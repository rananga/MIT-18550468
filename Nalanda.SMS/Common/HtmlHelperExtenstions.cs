    ...
    public static class WebHelpers
    {
        public static Tentity GetEntity<Tentity, Tmodel>(this IModel<Tentity, Tmodel> mapper)
        {
            ...
            using (var db = new dbNalandaContext())
            {
                ...
                if (pkProps.Count == 1 && pkProps[0].ClrType == typeof(int))
                {
                    var identProp = retTyp.GetProperties().FirstOrDefault(p => p.Name == pkProps[0].Name);

                    if (identProp != null)
                    {
                        var idVal = (int)identProp.GetValue(retObj);
                        if (idVal < 0)
                            identProp.SetValue(retObj, 0);
                    }
                }
            }
