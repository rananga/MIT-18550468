...
$(function () {
    ...
    var objPeriod2 = $('#PeriodID2');
    var objPeriodTO2 = $("#PeriodTo2");
    ...
    objPeriod.change(function () {
        ...
        objClass.empty().append('<option selected="selected" value="">' + objPromotion.data("empty-text") + '</option>');

    });

    objPeriod2.change(function () {

        $.getJSON(AppRoot + "Student/ClassRegistration/GetPeriodDetails", { PeriodID: Number(objPeriod2.val()) }, function (jsn) {
           objPeriodTO2.val(jsn.periodTo);
        }).error(function (data, status, jqXHR) {
            if (IsJson(data.responseText)) { AlertIt("ERROR: " + JSON.parse(data.responseText).Message); }
            else { AlertIt("ERROR: " + data.statusText); }
        });
